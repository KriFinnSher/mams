	...
	tests := []struct {
	...
	}{
		{
			...
			setup: func(m *mocks.MockServiceReader) *http.Request {
				...
				req := httptest.NewRequest(http.MethodGet, "/api/services/"+serviceID.String()+"/metrics", nil)
		...
		},
		{
			...
			setup: func(_ *mocks.MockServiceReader) *http.Request {
				req := httptest.NewRequest(http.MethodGet, "/api/services/bad-id/metrics", nil)

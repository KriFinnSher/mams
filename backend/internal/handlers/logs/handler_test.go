...
func TestGet_Success(t *testing.T) {
	...
	sl := mocks.NewMockServiceLister(ctrl)
	orgs := mocks.NewMockOrgGetter(ctrl)
	jwt := mocks.NewMockJWTValidator(ctrl)
	sid := uuid.New()
	lr.EXPECT().ListByService(gomock.Any(), sid, gomock.Any()).Return([]models.LogEntry{{ServiceID: sid, Message: "ok"}}, nil)
	h := NewHandler(lr, nil, svc, sl, orgs, ws.NewHub(), logx.New(slog.New(slog.NewTextHandler(io.Discard, nil))), jwt)
